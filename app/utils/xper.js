...
export const getErrorMessage = (error, response, body) => {
    const {t} = i18next;
    const xperParamsCheckMessage = !response || response.statusCode === 404 || response.statusCode === 401 ? t('global.alert_please_check_your_xper_parameters') : "";
        ...
        function (error, response, body) {
            ...
            if(error || !response || response.statusCode !== 200) {
                ...
                try {
                    ...
                    callback(null);
                    console.error(e);
                    remote.dialog.showErrorBox(t('global.error'), `${t('global.alert_bad_xper_3_response')} ${t('global.alert_please_check_your_xper_parameters')}`);
                }
            }
        }
    );
                    ...
                    .on('end', () => {
                        setTimeout(() => {
                            try {
                                ...
                                remote.dialog.showErrorBox(t('global.error'), t('global.alert_bad_format_for_xper_3_file'));
                            }
                        }, 100);
                    })
                    .on('error', () => {
                        console.log(error)
                        remote.dialog.showErrorBox(t('global.error'), getErrorMessage(error, null, null));
...
export const exportSddToDatabase = (sourceSdd, taxonomyInstance, selectedTaxonomy, pictures, database, callback) => {
    ...
    try {
        ...
        console.error(e);
        remote.dialog.showErrorBox(t('global.error'), t('global.alert_an_error_occurred_during_export_to_xper_database'));
    }
    if (fs.existsSync(filepath)) {
        ...
        console.error("file is not created", filepath);
        remote.dialog.showErrorBox(t('global.error'), t('global.alert_an_error_occurred_during_export_to_xper_database'));
    }
}
